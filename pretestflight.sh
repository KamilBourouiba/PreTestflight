...
write_ui_test_stub() {
  local out_file="$1"
  cat > "$out_file" << 'STUB'
// Generated by PreTestflight - UI test stubs (~15 tests; fill using prompts/ui_test_prompt.txt)
...
class PreTestflightUITests: XCTestCase {
    ...
    func test_navigationBarExists() throws {
        XCTAssertTrue(app.navigationBars.firstMatch.waitForExistence(timeout: 3))
    }
    func test_listOrScrollViewExists() throws {
        let list = app.tables.firstMatch
        let scroll = app.scrollViews.firstMatch
        XCTAssertTrue(list.waitForExistence(timeout: 2) || scroll.waitForExistence(timeout: 2))
    }
    func test_primaryButtonAccessibility() throws {
        let btn = app.buttons.firstMatch
        if btn.exists { XCTAssertTrue(btn.frame.width >= 44 || btn.frame.height >= 44, "Touch target") }
    }
    func test_noUnlabeledButtons() throws {
        for b in app.buttons.allElementsBoundByIndex where b.exists {
            XCTAssertTrue(b.label.isEmpty == false || b.identifier.isEmpty == false, "Button should have label or id")
        }
    }
    func test_rotation_portrait() throws {
        XCUIDevice.shared.orientation = .portrait
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 2))
    }
    func test_rotation_landscape() throws {
        XCUIDevice.shared.orientation = .landscapeLeft
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 2))
    }
    func test_backFromDetail() throws {
        if app.buttons["back"].exists { app.buttons["back"].tap(); XCTAssertTrue(app.wait(for: .runningForeground, timeout: 2)) }
    }
    func test_settingsScreenIfPresent() throws {
        if app.buttons["settings"].exists {
            app.buttons["settings"].tap()
            XCTAssertTrue(app.wait(for: .runningForeground, timeout: 3))
        }
    }
    func test_dynamicType_large() throws {
        app.launchArguments = ["-UIPreferredContentSizeCategory", "UIContentSizeCategoryAccessibilityExtraLarge"]
        app.launch()
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 5))
    }
    func test_modalDismissIfPresent() throws {
        if app.buttons["Done"].exists { app.buttons["Done"].tap() }
        if app.buttons["Cancel"].exists { app.buttons["Cancel"].tap() }
    }
    func test_searchFieldIfPresent() throws {
    ...
    func test_tabBarIfPresent() throws {
        if app.tabBars.firstMatch.waitForExistence(timeout: 2) {
            XCTAssertGreaterThan(app.tabBars.buttons.count, 0)
        }
    }
    func test_alertHandling() throws {
        addUIInterruptionMonitor(withDescription: "System Dialog") { _ in true }
    }
}
STUB
}
...
write_xctest_stub() {
  ...
  cat > "$out_file" << STUB
// Generated by PreTestflight - Unit test stubs (~15 tests; fill using prompts/xctest_prompt.txt)
// Product: PreTestflight, Author: Kamil Bourouiba
...
class PreTestflightUnitTests: XCTestCase {
    ...
    func test_initialState() throws {
        XCTAssertTrue(true, "Replace with initial state assertion")
    }
    func test_emptyInput_returnsExpected() throws {
        XCTAssertTrue(true, "Replace: empty input behavior")
    }
    func test_validInput_returnsSuccess() throws {
        XCTAssertTrue(true, "Replace: valid path")
    }
    func test_boundaryValue_min() throws {
        XCTAssertTrue(true, "Replace: min boundary")
    }
    func test_boundaryValue_max() throws {
        XCTAssertTrue(true, "Replace: max boundary")
    }
    func test_nilInput_handled() throws {
        XCTAssertTrue(true, "Replace: nil handling")
    }
    func test_invalidInput_throwsOrReturnsError() throws {
        XCTAssertTrue(true, "Replace: error path")
    }
    func test_asyncCompletes() throws {
        let exp = expectation(description: "async")
        DispatchQueue.main.async { exp.fulfill() }
        wait(for: [exp], timeout: 1)
    }
    func test_publishedValueUpdates() throws {
        XCTAssertTrue(true, "Replace: Combine @Published")
    }
    func test_teardownCleansState() throws {
        XCTAssertTrue(true, "Replace: teardown side effects")
    }
    func test_concurrentAccess_safe() throws {
        XCTAssertTrue(true, "Replace: thread safety")
    }
    func test_mockInjected_used() throws {
        XCTAssertTrue(true, "Replace: mock verification")
    }
    func test_dateFormatting() throws {
        XCTAssertTrue(true, "Replace: date formatter")
    }
    func test_localizedString_exists() throws {
        XCTAssertTrue(true, "Replace: localization key")
    }
    func test_permissionRequest_doesNotCrash() throws {
        XCTAssertTrue(true, "Replace: permission flow")
    }
}
STUB
}
